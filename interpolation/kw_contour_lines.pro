...
IF (N_ELEMENTS(dims) NE 2) THEN MESSAGE, 'Data mus be a 2-D array!'   ;Check data array is 2 dimensions
...
FOR i = 0, dims[0]-1 DO BEGIN                                         ;Iterate over x-values
  FOR j = 0, dims[1]-2 DO BEGIN                                       ;Iterate over y-values
    ...
    y_int = INTERPOL([j, j+1], z[i,j:j+1], level)                     ;Find y value where data array EQ level
    IF (y_int GE j AND y_int LE j+1) THEN BEGIN                       ;Index must be at or between y-values
      x_vals = [x_vals, x[i]]                                         ;Store x location
      y_vals = [y_vals, INTERPOLATE(y, y_int)]                        ;Determine y-value and store
    ENDIF 
  ENDFOR                                                              ;END j
ENDFOR                                                                ;END i
...
FOR i = 0, N_ELEMENTS(x_vals)-1 DO BEGIN                              ;Iterate over all values
  ...
  FOR j = 0, N_ELEMENTS(x_vals)-1 DO BEGIN                            ;Re-iterate to find neighboring points
    IF (j EQ i) THEN CONTINUE                                         ;Check if already used, or if at i iteration
    id = WHERE(j EQ already_used, check)
    IF (check NE 0) THEN CONTINUE
    
    id = WHERE(tmp_x[-1]+x_skip GE x_vals AND $
               tmp_x[-1]        LT x_vals AND $
               tmp_y[-1]+y_skip GE y_vals AND $
               tmp_y[-1]-y_skip LE y_vals, CNT)
    IF (CNT NE 0) THEN BEGIN                                          ;If a nearby point is found, append to arrays
      tmp_x = [tmp_x, x_vals[id]] 
      tmp_y = [tmp_y, y_vals[id]] 
      already_used = [already_used, id]                               ;Append these indices to already used array
    ENDIF
  ENDFOR                                                              ;END j
  ...
  lines = CREATE_STRUCT(lines, 'LINE_'+STRTRIM(line_cnt,2), $         ;Append contour to structure
                               {X_VALUES : tmp_x, $
                                Y_VALUES : tmp_y}) 
