FUNCTION KW_EOF_SVD_V01, inArray, $
...
IF dims[0] GT dims[1] THEN BEGIN																								; If first dimension in larger
	array = TRANSPOSE(array)																											; Transpose the array
	dims  = REVERSE(dims, /OVERWRITE)																							; Reverse the dimensions
ENDIF

nvariables = KEYWORD_SET(nVariables) eq 0 ? dims[1] : LONG(nVariables)					; Set number of variables to return

LA_SVD, array, S, U, V																													; Perform singular value decomposition
...
FOR i = 0, dims[0]-1 DO BEGIN																										; Iterate over first dimension
	temp_var          = MATRIX_MULTIPLY(array, U[i,*], /BTRANSPOSE) 							; Compute input array times u
	eigenvectors[0,i] = temp_var / SQRT(TOTAL(temp_var^2))												; Compute eigenvectors
	a_t[0,i]          = MATRIX_MULTIPLY(array, eigenvectors[*,i], /ATRANSPOSE)		; Compute a_t
	eigenvalues[i]    = CORRELATE(a_t[*,i], a_t[*,i], /COVARIANCE)								; Compute eigenvalues
ENDFOR
eigenvectors = TRANSPOSE(eigenvectors)																					; Transpose the eigenvectors
variance     = EIGENVALUES / TOTAL(EIGENVALUES)																	; Compute variance
