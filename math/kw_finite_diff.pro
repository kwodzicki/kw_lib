FUNCTION KW_FINITE_DIFF, z_in, x_in, y_in, $ 
...
IF (SIZE(y, /N_DIMENSIONS) EQ 1) THEN y = REBIN(REFORM(y, 1, dims[1]), dims)
...
x = TEMPORARY(new_x)
new_y = MAKE_ARRAY(dims+nCo*2, VALUE=!VALUES.D_NaN)
new_y[0,     nCo] = y[-1*nCo:-1,*]
new_y[nCo,   nCo] = y
new_y[-1*nCo,nCo] = y[0:nCo-1,*]
new_y[nCo,0]      = REVERSE(SHIFT(y[*,1:nCo],       sp),2)
new_y[nCo,-1*nCo] = REVERSE(SHIFT(y[*,-1*nCo-1:-2], sp),2)
y = TEMPORARY(new_y)
