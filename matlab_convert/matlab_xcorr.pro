FUNCTION MATLAB_XCORR, X, Y, SCALEOPT = scaleoptIn, $
...
IF N_ELEMENTS(lag) EQ 0 THEN lag = LINDGEN(nX * 2 - 1) - (nX - 1)		  					; Generate lag values

scaleopt = N_ELEMENTS(scaleoptin) EQ 0 ? 'NONE' : STRUPCASE(scaleoptin)                   ; Set default scaleplot value OR convert to uppercase
...
FOR k = 0L, nLag-1 DO $
	Cross[k] = (Lag[k] GE 0) ? $																									; Note the reversal of the variables for negative lags.
		TOTAL(Xd[Lag[k]:*] * Yd[0:nX - Lag[k] - 1L],  Double = useDouble) : $
		TOTAL(Xd[0:nX + Lag[k] - 1L] * Yd[-Lag[k]:*], Double = useDouble)

idZero = WHERE(Lag EQ 0, CNT)
CASE scaleopt OF
    'BIASED'    : Cross = TEMPORARY(Cross) / nX
    'UNBIASED'  : Cross = TEMPORARY(Cross) / (nX - ABS(lag))
    'COEFF'     : Cross = TEMPORARY(Cross) / SQRT( TOTAL(Xd^2)*TOTAL(Yd^2) )
;    'COEFF'     : Cross = TEMPORARY(Cross) / SQRT( Xd[idZero] * Yd[idZero] )
	ELSE        : ; Do nothting
ENDCASE
