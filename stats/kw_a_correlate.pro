; $Id: //depot/Release/ENVI52_IDL84/idl/idldir/lib/a_correlate.pro#1 $
...
;-
function KW_A_Correlate, X, Lag, Covariance = Covariance, Double = doubleIn, NaN = nan
    ...
    ;
    data = X - (TOTAL(X, Double = useDouble, NaN=NaN) / nX)

    ;Compute Autocovariance.
    M = ABS(Lag)
    for k = 0L, nLag-1 do $
        Auto[k] = TOTAL(data[0:nX - 1 - M[k]] * data[M[k]:*], NaN = NaN)

    ; Divide by N for autocovariance, or by variance for autocorrelation.
    Auto = TEMPORARY(Auto) / $
        (KEYWORD_SET(Covariance) ? nX : TOTAL(data^2, NaN=NaN))
