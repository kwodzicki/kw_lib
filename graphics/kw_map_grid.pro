...
IF !MAP.LL_BOX[1] LT 180 THEN $
  lonBounds = LIST( [!MAP.LL_BOX[1], 180], [180, !MAP.LL_BOX[3]]) $
ELSE $
  lonBounds = LIST( [!MAP.LL_BOX[3], 180], [180, !MAP.LL_BOX[1]])
...
WHILE lat LT 90 DO BEGIN
  FOR i = -1, 1, 2 DO BEGIN
    ...
    IF llat GE !MAP.LL_BOX[0] AND llat LE !MAP.LL_BOX[2] THEN BEGIN
      ...
      FOREACH lon, lonBounds DO $
        PLOTS, lon, llat*[1,1], /DATA, $
          LINESTYLE = extra['GLINESTYLE']
      IF KEYWORD_SET(label) THEN BEGIN 
        xy = CONVERT_COORD(!MAP.LL_BOX[1], llat, /DATA, /TO_NORMAL)
        XYOUTS, $
          xy[0]-!X_CH_SIZE*extra['CHARSIZE']*0.5, $
          xy[1]-!Y_CH_SIZE*extra['CHARSIZE']*0.5, $
          STRTRIM(LONG(llat),2), $
          CHARSIZE  = extra['CHARSIZE'], $
